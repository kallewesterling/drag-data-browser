...
let originalGraphNodes;
let originalGraphEdges;
let originalGraph = {};
let partitionInit;
let edgeIndex = {};
...
function getDegreeForNode(graph, node) {
...
function getNeighboursOfNode(graph, node) {
...
function add_edge_to_graph(graph, edge) {
  ...
  if (edgeIndex[edge.source + '_' + edge.target]) {
    graph.edges[edgeIndex[edge.source + '_' + edge.target]].weight = edge.weight;
  } else {
    graph.edges.push(edge);
    edgeIndex[edge.source + '_' + edge.target] = graph.edges.length - 1;
  }
}
...
function init_status(graph, status, part) {
  ...
  if (typeof part === 'undefined') {
    graph.nodes.forEach(function(node, i) {
      ...
      let deg = getDegreeForNode(graph, node);
    ...
    graph.nodes.forEach(function(node, i) {
      ...
      status.gdegrees[node] = deg;
      let inc = 0.0;
      let neighbours = getNeighboursOfNode(graph, node);
...
function __modularity(status) {
  ...
  let result = 0.0;
  const communities = makeSet(objValues(status.nodes_to_com));
...
function __neighcom(node, graph, status) {
  ...
  // node_to_com
  const weights = {};
  const neighboorhood = getNeighboursOfNode(graph, node); // make iterable;
...
function induced_graph(partition, graph) {
  ...
  const partition_values = objValues(partition);
  ret.nodes = ret.nodes.concat(makeSet(partition_values)); // make set

  graph.edges.forEach(function(edge, i) {
    weight = edge.weight || 1;
    const com1 = partition[edge.source];
    const com2 = partition[edge.target];
    w_prec = getEdgeWeight(ret, com1, com2) || 0;
...
function generate_dendogram(graph, part_init) {
  ...
  init_status(originalGraph, status, part_init);
  let mod = __modularity(status);
  const status_list = [];
  __one_level(originalGraph, status);
  let new_mod = __modularity(status);
  let partition = __renumber(status.nodes_to_com);
  status_list.push(partition);
  mod = new_mod;
  let current_graph = induced_graph(partition, originalGraph);
...
(function(root, factory) {
  ...
  return function() {
    const core = function() {
      const dendogram = generate_dendogram(originalGraph, partitionInit);
    ...
    core.edges = function(edgs) {
      if (typeof originalGraphNodes === 'undefined') {
        throw new TypeError('Graph nodes are undefined');
      }

      if (arguments.length > 0) {
        originalGraphEdges = edgs;
        let assoc_mat = make_assoc_mat(edgs);
        originalGraph = {
          nodes: originalGraphNodes,
          edges: originalGraphEdges,
