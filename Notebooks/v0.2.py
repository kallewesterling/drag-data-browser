...
df = df.drop(df[df['filter'] == 'unsure_exclude'].index)

# %%
# Drop duplicates based on subset of columns
df = df.drop_duplicates(subset=['Date', 'Performer', 'Venue', 'City', 'State', 'Source'])
...
# %%
# Drop filtered data
df = df.drop(df[df['filter']].index)


# %%
# Set up helpful columns
def has(row, column):
    return row[column] != '' and \
           row[column] != '?' and \
           not row[column].startswith('—') and \
           not row[column].startswith('(')


df['has_unique_venue'] = df.apply(lambda row: has(row, 'Unique venue'), axis=1)
df['has_performer'] = df.apply(lambda row: has(row, 'Performer'), axis=1)
df['has_city'] = df.apply(lambda row: has(row, 'City'), axis=1)
df['has_revue'] = df.apply(lambda row: has(row, 'Revue'), axis=1)

# %%
# Drop columns
df = df.drop_columns(drop_columns)
...
PERFORMER_COMMENTS = reverse_comment_dict(performer_comments)
VENUE_COMMENTS = reverse_comment_dict(venue_comments)
CITY_COMMENTS = reverse_comment_dict(city_comments)

# %%

# %%
# Add jinja functions to globals
Settings.e.globals['get_venue_slug'] = lambda venue: ALL_VENUES[venue]
Settings.e.globals['get_performer_slug'] = lambda performer: ALL_PERFORMERS[performer]

# Add jinja "constants" to globals
set_globals(Settings.e, ALL_YEARS)
set_globals(Settings.e, ALL_PERFORMERS)
set_globals(Settings.e, ALL_VENUES)
set_globals(Settings.e, ALL_CITIES)

# Add comments to jinja globals
set_globals(Settings.e, PERFORMER_COMMENTS)
set_globals(Settings.e, VENUE_COMMENTS)
set_globals(Settings.e, CITY_COMMENTS)

# Add all newspapers.com and EIMA ids to the globals
set_globals(Settings.e, NEWSPAPER_LINKS)
set_globals(Settings.e, EIMA_LINKS)
...
def lookup_footnote(footnote, footnote_data=footnote_dir, get_field='fullReference'):
    footnote_looked_up = footnote_data.get(footnote, {})
...
individual_networks_path_name = 'v1-co-occurrence-grouped-by-14-days-no-unnamed-performers'
individual_networks_dir = os.path.join(input_path, 'data', 'individual-networks', individual_networks_path_name)
...
filtered_data_files = [x for x in data_files if 'co-occurrence' not in x.name and not x.name.startswith('.')]
...
def collect_similar_names(name_json):
    similar_names = []
    collected = []
    for x, y, z, aa, bb in name_json:
...
def get_case_study_data(number=None, case_study_titles=Settings.CASE_STUDY_TITLES):
    ...
    for section in chapter.get('sections', []):
        ...
        for ix, paragraph in enumerate(section['paragraphs']):
            if paragraph.endswith('.png'):
                img_src = '{Settings.BASE_URL}data/case-study-{number}/{section["paragraphs"][ix]}'
                section['paragraphs'][ix] = f'<img src="{img_src}" class="border shadow-sm rounded-3" />'
    ...
    for section in chapter.get('sections', []):
        for ix, para in enumerate(section['paragraphs']):
            for found_artist in ARTISTS.findall(para):
                try:
                    performer_slug = Settings.e.globals['get_performer_slug'](found_artist)
                except KeyError:
                    performer_slug = None
            ...
            for footnote in FOOTNOTES.findall(para):
                ...
                html = f'<sup>' \
                       f'<a id="inlineFootnote{len(all_footnotes)}" ' \
                       f'   href="#fn{len(all_footnotes)}" ' \
                       f'   class="text-decoration-none text-secondary">' \
                       f'{len(all_footnotes)}' \
                       f'</a>' \
                       f'</sup>'
                section['paragraphs'][ix] = section['paragraphs'][ix].replace(f'[fn={orig_footnote}]', html)
    ...
    case_study_data = {
        ...
        'footnotes': {
            footnoteCount: footnote for footnoteCount, footnote in enumerate(all_footnotes, start=1)
        },
    }
...
meta_case_study_data = {
    ...
    'subtitle': '(Re)Constructing a Network of Itinerant Queer Nightlife Performers in the 1930s'
                'Through Data-Assisted Historiography',
...
write_html(Settings.e,
           'case-study/meta.html',
           data=meta_case_study_data,
           path=('case-study', 'theorizing-the-peripatetic'))
