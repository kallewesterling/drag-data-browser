...
def get_basic_df(get_filter=False, year_range=YEAR_RANGE):
    '''
    ...
    def filtered(row, year_range=year_range):
        '''
        Takes a DataFrame's `row` and returns False if the row does NOT contains disqualifying information:
            ...
            (5) The date's year not within year_range
    ...
    normalization_columns = ['Source clean', 'Normalized performer', 'Performer first-name', 'Performer last-name', 'Normalized City', 'Normalized Revue Name', 'Normalized Venue', 'EIMA_ID', 'EIMA_Search', 'Newspapers_ID', 'Search (newspapers.com)']
...
def get_comments(df, comment_field='Comment on edge: revue', match_field='Revue', transform=None):
    ...
    warnings = []
    for date, source, match, comment in zip(rows_with_comments['Date'], rows_with_comments['Source'], rows_with_comments[match_field], rows_with_comments[comment_field]):
